...
const Store = () => {
  ...
  const filteredBooks = books.filter((book)=>{
    const matchesCategory = filteredCategory === "All" ||  book.category === filteredCategory
                ...
                <div key={book?.id}>
                  <div className="slide bg-blue-300 lg:w-auto w-[270px] rounded-xl flex items-center flex-col pt-3 ">
                    <div className="w-fit p-2 rounded-full">
                      <img src={book.img_url} alt="image" className="w-20 h-[150px]" />
                    </div>
                    <div className="bg-white lg:w-auto w-[200px] m-1 mt-5 rounded-xl p-4">
                      <div className="flex justify-between gap-4">
                        <h2 className="font-medium text-[18px]">{book?.name}</h2>
                        <span className="text-blue-500">₹{book.price}</span>
                      </div>
                      <h3 className="font-normal text-[14px]">{book.description}</h3>
                      <button onClick={()=>handleAddToCart(book)} className="mt-4 p-2 text-[15px] rounded-xl border border-blue-400">
                        Add To Cart
                      </button>
                    </div>
                  </div>
